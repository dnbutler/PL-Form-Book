...
export async function savePrediction(params: {
    ...
    .upsert(
      {
        ...
        home_total: output.home.total,
        away_total: output.away.total,
        edge: output.edge,
        home_prob: output.probabilities.homeProb,
        draw_prob: output.probabilities.drawProb,
        away_prob: output.probabilities.awayProb,
        verdict: output.verdict,
        confidence: output.confidence,
        rationale: output.rationale,
      },
      { onConflict: "fixture_id,model_version_id" }
    )
    .select("id")
    .single();
  ...
  if (predictionError || !prediction) {
    throw new Error(
      `Could not save prediction for fixture ${fixtureId}: ${predictionError?.message ?? "No row returned"}`
    );
  }
  ...
  const { error: deleteError } = await db
    .from("prediction_factor_scores")
    .delete()
    .eq("prediction_id", predictionId);

  if (deleteError) {
    throw new Error(
      `Could not clear factor scores for prediction ${predictionId}: ${deleteError.message}`
    );
  }

  const factorRows = [
    ...output.home.factors.map((factor) => ({
    ...
    ...output.away.factors.map((factor) => ({
      prediction_id: predictionId,
      team_id: awayTeamId,
      factor_key: factor.factorKey,
      factor_name: factor.factorName,
  ...
  if (factorRows.length === 0) {
    throw new Error(`No factor rows generated for prediction ${predictionId}`);
  }

  const { error: insertFactorError } = await db
    .from("prediction_factor_scores")
    .insert(factorRows);
